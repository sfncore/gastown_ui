...
> **Previous Version**: [INTEGRATION_PLAN_V5.md](./INTEGRATION_PLAN_V5.md)
> **Grounding Sources**: `../gastown` (CLI behavior), competing architecture plans
> **Performance Audit**: [GASTOWN_CLI_PERFORMANCE_AUDIT.md](./GASTOWN_CLI_PERFORMANCE_AUDIT.md)
...
### CLI Performance Baselines (v5.1)

> **Source**: [GASTOWN_CLI_PERFORMANCE_AUDIT.md](./GASTOWN_CLI_PERFORMANCE_AUDIT.md) (2026-01-12)

Measured CLI command latencies that define our performance envelope:

| Command | Mean Latency | Notes |
|---------|-------------|-------|
| `gt status --json` | **1260ms** | Critical bottleneck - called from 8 routes |
| `gt status --fast --json` | **1020ms** | Skips mail lookups - **USE THIS** |
| `bd list --json` | 70ms | Acceptable |
| `gt convoys --json` | 15ms | Fast |
| `bd list --type=message --json` | 75ms | Mail queries |

**Key Finding**: Dashboard latency is dominated by `gt status` (1.26s). The `--fast` flag saves ~240ms by skipping per-agent mail lookups.

**Performance Targets**:

| Metric | Current | Target | Strategy |
|--------|---------|--------|----------|
| `gt status` response | 1260ms | <200ms | Backend caching (see audit) |
| Dashboard initial load | ~2.5s | <500ms | Use `--fast`, add SWR cache |
| Cached page navigation | ~1.3s | <100ms | 10s SWR TTL |

**Immediate Actions**:
1. Switch all routes to `gt status --fast --json`
2. Add 10-second SWR cache for status responses
3. Add composite `/api/gastown/snapshot` endpoint
...
// Command-specific timeouts (based on measured baselines + 2x headroom)
// See: docs/GASTOWN_CLI_PERFORMANCE_AUDIT.md for baseline measurements
const TIMEOUTS: Record<string, number> = {
  'gt status': 3000,       // Measured: ~1260ms (use --fast for ~1020ms)
  'gt status --fast': 2500, // Measured: ~1020ms - PREFER THIS
  'gt doctor': 120000,
  'gt rig add': 180000,
  'bd list': 500,          // Measured: ~70ms
  'bd show': 500,
  'gt convoys': 500,       // Measured: ~15ms
  'gt mail inbox': 10000,
  default: 30000
};
...
> **v5.1 Update**: Use `--fast` flag for `gt status` to skip mail lookups (~240ms faster).
> See [CLI Performance Baselines](#cli-performance-baselines-v51) for measured latencies.

| Route | Method | CLI Command | Response | Timeout | Cache TTL |
|-------|--------|-------------|----------|---------|-----------|
| `/api/gastown/snapshot` | GET | Composite (see below) | `GtSnapshot` | 5s | **10s** |
| `/api/gastown/status` | GET | `gt status --fast --json` | `GtStatus` | 3s | 10s |
| `/api/gastown/agents` | GET | Parse from status | `GtAgent[]` | 3s | 10s |
| `/api/gastown/agents/[id]` | GET | `gt peek <id>` | `GtAgentDetail` | 30s | 5s |
| `/api/gastown/convoys` | GET | `gt convoy list --json` | `GtConvoy[]` | 1s | 10s |
| `/api/gastown/convoys/[id]` | GET | `gt convoy show <id>` | `GtConvoy` | 30s | 5s |
| `/api/gastown/mail` | GET | `bd list --type=message` | `GtMailMessage[]` | 1s | 5s |
| `/api/gastown/mail/[id]` | GET | `bd show <id>` | `GtMailMessage` | 1s | 5s |
| `/api/gastown/queue` | GET | `gt mq list --json` | `GtMergeQueueItem[]` | 30s | 10s |
| `/api/gastown/rigs` | GET | `gt rig list --json` | `GtRig[]` | 30s | 30s |
| `/api/gastown/rigs/[name]` | GET | `gt rig show <name>` | `GtRig` | 30s | 10s |
| `/api/gastown/rigs` | POST | `gt rig add ...` | `{ id, status }` | **180s** | - |
| `/api/gastown/work/issues` | GET | `bd list --type=task` | `BdBead[]` | 1s | 10s |
| `/api/gastown/work/issues` | POST | `bd create ...` | `BdBead` | 30s | - |
| `/api/gastown/work/issues/[id]` | GET | `bd show <id>` | `BdBead` | 1s | 5s |
| `/api/gastown/work/sling` | POST | `gt sling ...` | `{ status }` | 60s | - |
| `/api/gastown/feed` | GET | `.events.jsonl` | `GtFeedItem[]` | 30s | 2s |
| `/api/gastown/feed/stream` | GET | SSE | `GtFeedItem` stream | N/A | - |
| `/api/gastown/health` | GET | `gt doctor --json` | `GtDoctorResult` | 90s | 60s |

#### Composite Snapshot Endpoint (v5.1)

The `/api/gastown/snapshot` endpoint reduces dashboard load time by combining multiple queries:

```typescript
// GET /api/gastown/snapshot
// Combines: status + mail count + queue count + recent activity
interface GtSnapshot {
  status: GtStatus;           // From: gt status --fast --json
  mailCount: number;          // From: bd list --type=message --status=open --json | .length
  queueCount: number;         // From: gt mq list --json | .length
  recentActivity: GtFeedItem[]; // From: tail -100 .events.jsonl
  freshness: {
    fetchedAt: string;
    ttlMs: number;
  };
...
- Reduces dashboard P95 from ~2.5s to ~1.2s (first load) / <100ms (cached)

#### Additional Workflow Routes

| Route | Method | CLI Command | Response | Timeout | Cache TTL |
|-------|--------|-------------|----------|---------|-----------|
| `/api/gastown/workflows/formulas` | GET | `gt formula list` | `GtFormula[]` | 30s | 30s |
| `/api/gastown/workflows/molecules` | GET | `bd list --type=molecule` | `GtMolecule[]` | 1s | 10s |
